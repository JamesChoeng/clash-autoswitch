...
> **PATH 配置**：`pipx`/`uvx` 会自动把命令加入 PATH。用 `pip install --user` 时，控制台脚本可能落在不在 PATH 上的目录里 —— 第一次执行 `clashpilot up` / `clashpilot hook` 会自动把该目录写入用户 PATH（Windows 还会安装一个 PowerShell 垫片，让 `clp` 覆盖内置同名别名）。也可随时手动执行 `clashpilot setup-path`。配置后需**新开一个终端**才能直接使用 `clashpilot` / `clp`。
...
> 嫌 `clashpilot` 太长？装好后会同时提供一个短命令 `clp`，与 `clashpilot` 完全等价，例如 `clp up`、`clp status`。
...
clashpilot setup-path  # 把 clashpilot/clp 脚本目录写入用户 PATH（幂等）
```
